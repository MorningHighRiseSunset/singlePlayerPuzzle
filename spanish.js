...
class ScrabbleGame {
	constructor() {
		...
		this.preferredLang = 'es'; // Spanish version
...
formedWords.forEach((wordInfo) => {
	...
	async init() {
		...
		try {
			// Load Spanish dictionary only for Spanish version
	...
	async loadSpanishDictionary() {
		try {
			...
			// Filter valid Scrabble words (only Spanish letters, reasonable length, no archaic/obscure words)
			...
			// Filter to create a proper Scrabble dictionary
			validWords = validWords.filter(word => {
				const lowerWord = word.toLowerCase();

				// Remove banned words (pronouns, articles, prepositions, conjunctions, archaic words)
				const bannedWords = new Set([
					// Archaic and invalid Scrabble words
					'trovar', 'rollen', 'antier', 'ayer', 'hoy', 'mañana',
					// Articles
					...
					'que', 'quien', 'quienes', 'cual', 'cuales', 'como', 'donde', 'cuando', 'cuanto', 'cuanta', 'cuantos', 'cuantas',
					// Prepositions
					'a', 'al', 'de', 'del', 'en', 'por', 'para', 'con', 'sin', 'sobre', 'tras', 'durante', 'desde', 'hasta',
					'contra', 'hacia', 'mediante', 'excepto', 'salvo', 'incluso',
					// Conjunctions
					'y', 'o', 'u', 'e', 'ni', 'pero', 'mas', 'sino', 'aunque', 'porque', 'pues', 'luego',
					// Adverbs that are too basic
					'si', 'no', 'tal', 'tan', 'tanto', 'muy', 'poco', 'mucho', 'bastante', 'demasiado',
					'siempre', 'nunca', 'nada', 'todo', 'todos', 'todas', 'otro', 'otra', 'otros', 'otras',
					'mismo', 'misma', 'mismos', 'mismas',
					// Common verbs in conjugated forms that are too basic
					'es', 'son', 'era', 'eran', 'fue', 'fueron', 'sea', 'sean', 'estoy', 'estas', 'esta', 'estamos', 'estan',
					'estaba', 'estabas', 'estaba', 'estabamos', 'estaban', 'estuve', 'estuviste', 'estuvo', 'estuvimos', 'estuvieron'
				]);

				if (bannedWords.has(lowerWord)) return false;

				// Remove words that are too long for Scrabble (15x15 board max ~11-12 letters realistically)
				if (word.length > 11) return false;

				// Remove conjugated verb forms that are not useful for Scrabble
				// Keep infinitives, gerunds, and past participles that can form nouns/adjectives
				if (/^(soy|eres|es|somos|sois|son)$/.test(lowerWord)) return false; // ser conjugations
				if (/^(estoy|estas|esta|estamos|estais|estan)$/.test(lowerWord)) return false; // estar conjugations
				if (/^(tengo|tienes|tiene|tenemos|teneis|tienen)$/.test(lowerWord)) return false; // tener conjugations

				// Remove diminutives that are too obscure (keep some common ones)
				if (lowerWord.endsWith('ito') || lowerWord.endsWith('ita') ||
					lowerWord.endsWith('illo') || lowerWord.endsWith('illa') ||
					lowerWord.endsWith('ico') || lowerWord.endsWith('ica')) {
					// Only keep diminutives of common words
					const baseWord = lowerWord.replace(/(ito|ita|illo|illa|ico|ica)$/, '');
					const commonBases = ['pan', 'mes', 'cas', 'perr', 'gat', 'libr', 'cuchill', 'camis'];
					if (!commonBases.some(base => baseWord.includes(base))) return false;
				}

				// Remove words with too many rare letters (Spanish Scrabble has limited J,K,Q,W,X,Y,Z)
				const rareLetters = (lowerWord.match(/[jkqwxyz]/g) || []).length;
				if (rareLetters > 2) return false;

				return true;
			});

			// Limit to reasonable Scrabble dictionary size (~50k-80k words)
			if (validWords.length > 80000) {
				validWords = validWords.slice(0, 80000);
			}
			...
			console.log(`Spanish Scrabble dictionary loaded: ${this.spanishDictionary.size} words (${this.spanishDictionaryNormalized.size} normalized)`);
		} catch (error) {
	...
	async loadMandarinDictionary() {
		try {
			...
			this.mandarinDictionary = new Set(words);
			console.log(`Mandarin dictionary loaded: ${this.mandarinDictionary.size} words`);
		} catch (error) {
