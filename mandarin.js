...
class ScrabbleGame {
	constructor() {
		...
		// Mandarin tile values and distribution (example, adjust as needed)
		this.tileValues = {
			'的': 1, '一': 1, '是': 2, '在': 2, '不': 2, '了': 2, '有': 2, '人': 2, '这': 2, '中': 2,
			'我': 2, '以': 2, '要': 2, '他': 2, '时': 2, '来': 2, '用': 2, '们': 2, '生': 2, '到': 2,
			'大': 2, '小': 2, '好': 2, '上': 2, '下': 2, '天': 2, '地': 2, '国': 2, '家': 2, '学': 2,
			'校': 2, '工': 2, '作': 2, '朋': 2, '友': 2, '老': 2, '师': 2, '孩': 2, '子': 2, '父': 2,
			'母': 2, '汽': 2, '车': 2, '城': 2, '市': 2, '幸': 2, '福': 2, '美': 2, '丽': 2, '*': 0
		};
		this.tileDistribution = {
			'的': 10, '一': 10, '是': 8, '在': 8, '不': 8, '了': 8, '有': 8, '人': 8, '这': 8, '中': 8,
			'我': 8, '以': 8, '要': 8, '他': 8, '时': 8, '来': 8, '用': 8, '们': 8, '生': 8, '到': 8,
			'大': 8, '小': 8, '好': 8, '上': 8, '下': 8, '天': 8, '地': 8, '国': 8, '家': 8, '学': 8,
			'校': 8, '工': 8, '作': 8, '朋': 8, '友': 8, '老': 8, '师': 8, '孩': 8, '子': 8, '父': 8,
			'母': 8, '汽': 8, '车': 8, '城': 8, '市': 8, '幸': 8, '福': 8, '美': 8, '丽': 8, '*': 4
		};
	...
	async getWordDefinition(word) {
		const definition = await fetchWordDefinition(word, "zh");
	...
	checkAIMoveValidity(word, startPos, isHorizontal) {
		// List of variant spellings or non-Chinese words to exclude
		const excludedVariants = new Set([
			"atropin", // German spelling, not valid in Chinese Scrabble
	...
	async loadDictionary() {
		...
		try {
			...
			const cleanedWords = words
				.filter(w => w && typeof w === 'string' && w.length >= 2 && w.length <= 7)
				.map(w => w.trim())
				.filter(w => /^[\u4e00-\u9fff]+$/.test(w));
			...
			"颜色", "红", "黄", "蓝", "绿", "黑", "白", "紫", "灰", "棕", "金", "银", "粉红", "橙", "深红",
			"感觉", "美", "丑", "甜", "苦", "酸", "咸", "辣", "香", "臭", "软", "硬",
			"热", "冷", "干", "湿", "粗", "细", "厚", "薄", "长", "短", "宽", "窄",
			"高", "低", "快", "慢", "强", "弱", "多", "少", "好", "坏", "新", "旧",
			"亮", "暗", "清", "浊", "轻", "重", "贵", "贱", "满", "空", "真", "假",
			"数字", "一", "二", "三", "四", "五", "六", "七", "八", "九", "十", "百",
			"千", "万", "亿", "兆", "零", "加", "减", "乘", "除", "等于", "大于", "小于",
			"几何", "线", "面", "体", "角", "圆", "方", "三角", "四边", "五边", "六边", "七边",
			"时间", "年", "月", "日", "时", "分", "秒", "毫秒", "周一", "周二", "周三", "周四",
			"周五", "周六", "周日", "春", "夏", "秋", "冬", "昨天", "今天", "明天", "后天", "前天",
			"早", "中", "晚", "夜", "黎明", "日出", "日落", "黄昏", "午夜", "中午", "傍晚", "宵夜",
			"星期", "节日", "假期", "休息", "睡眠", "清醒", "梦", "醒", "打哈欠", "打鼾",
			"头", "脸", "脸皮", "额头", "眉毛", "眼睑", "睫毛", "鼻子", "鼻孔", "鼻翼",
			"唇", "舌", "牙", "牙齿", "牙龈", "咽喉", "喉咙", "气管", "食管", "胸", "胸部",
			"腹", "腰", "背", "脊", "脊椎", "肋", "肋骨", "腹肌", "脐", "脐带", "肚脐",
			"手", "手指", "大拇指", "食指", "中指", "无名指", "小指", "掌", "掌心", "手心",
			"腕", "腕关节", "前臂", "肘", "肘关节", "上臂", "肩", "肩膀", "臂", "脚", "脚趾",
			"脚心", "脚掌", "脚踝", "踝", "小腿", "膝", "膝盖", "膝关节", "大腿", "股", "臀", "臀部",
			"器官", "心", "肺", "肝", "肾", "胃", "肠", "小肠", "大肠", "胆", "胆囊", "脾",
			"膵", "胰", "膀胱", "子宫", "卵巢", "睾丸", "大脑", "小脑", "脑干", "脊髓", "神经",
			"血管", "动脉", "静脉", "毛细血管", "血液", "血红蛋白", "红细胞", "白细胞", "血小板", "淋巴",
			"淋巴液", "淋巴节", "扁桃体", "胸腺", "脑垂体", "松果体", "甲状腺", "副甲状腺", "肾上腺",
			"皮肤", "皮肤细胞", "表皮", "真皮", "皮下", "汗腺", "汗孔", "油脂腺", "毛囊", "毛", "须",
			"胡须", "胡子", "眉", "眉毛", "头发", "发质", "发色", "发根", "发梢", "指甲", "脚趾甲",
			"脸颊", "脸蛋", "酒窝", "下巴", "颏", "喉结", "喉头", "颈", "脖子", "脖颈",
			"职业", "医生", "医护", "护士", "医院", "诊所", "手术", "治疗", "药物", "针", "针灸",
			"教师", "老师", "教育", "学校", "学生", "功课", "考试", "成绩", "证书", "教科书",
			"律师", "法律", "法官", "法院", "案件", "审判", "判决", "法律条款", "证人", "证据",
			"工程师", "工程", "建筑", "设计", "结构", "图纸", "施工", "机械", "设备", "机器",
			"农民", "农业", "农场", "耕种", "收获", "庄稼", "谷物", "粮食", "肥料", "灌溉",
			"工人", "工厂", "生产", "制造", "加工", "组装", "检查", "质量", "效率", "工作",
			"商人", "商业", "贸易", "买卖", "商店", "商品", "价格", "销售", "利润", "财务",
			"艺术", "艺术家", "绘画", "素描", "雕塑", "版画", "展览", "画廊", "色彩", "线条",
			"音乐", "音乐家", "歌唱", "乐器", "钢琴", "吉他", "小提琴", "管乐", "舞蹈", "编舞",
			"体育", "运动", "运动员", "球", "足球", "篮球", "乒乓球", "网球", "羽毛球", "排球",
			"游泳", "跑步", "跳高", "跳远", "摔跤", "拳击", "武术", "健身", "体操", "冰滑",
			"科学", "物理", "化学", "生物", "地理", "天文", "地质", "气象", "环境", "生态",
			"数学", "几何", "代数", "微积分", "统计", "概率", "方程", "公式", "定理", "证明",
			"技术", "计算机", "软件", "硬件", "程序", "代码", "网络", "互联网", "数据", "安全",
			"交通", "汽车", "火车", "飞机", "船", "自行车", "摩托车", "公交", "地铁", "轮船",
			"旅游", "旅行", "旅游景点", "酒店", "旅馆", "度假", "旅游指南", "护照", "签证", "机票",
			"文化", "文学", "诗歌", "小说", "散文", "戏剧", "电影", "电视", "摄影", "报纸",
			"历史", "古代", "现代", "中世纪", "文明", "帝国", "革命", "战争", "和平", "条约",
			"地理", "地球", "大陆", "国家", "城市", "地区", "地形", "气候", "环境", "资源",
			"植物学", "植物", "草本", "木本", "草地", "森林", "灌木林", "沙漠", "湿地", "沼泽",
			"动物学", "动物", "哺乳动物", "鸟类", "爬行动物", "两栖动物", "鱼类", "昆虫", "节肢动物", "软体动物",
			"天气", "晴", "云", "阴", "雨", "雪", "风", "雹", "雷", "闪电", "虹", "台风",
			"季节", "春季", "夏季", "秋季", "冬季", "春天", "夏天", "秋天", "冬天", "季节交替",
			"情感", "快乐", "悲伤", "生气", "害怕", "惊讶", "厌恶", "骄傲", "谦虚", "嫉妒",
			"感觉", "高兴", "难过", "开心", "伤心", "愉快", "烦恼", "痛苦", "喜悦", "失望",
			"动作", "行走", "跑", "跳", "爬", "游", "飞", "落", "坐", "站", "躺", "卧",
			"状态", "出生", "成长", "成熟", "衰老", "死亡", "健康", "疾病", "伤害", "恢复",
			"交流", "说", "听", "读", "写", "讲", "唱", "笑", "哭", "喊", "叫", "呼",
			"认知", "学习", "思考", "记忆", "忘记", "理解", "知道", "不知道", "相信", "怀疑",
			"关系", "家族", "父亲", "母亲", "兄弟", "姐妹", "儿子", "女儿", "丈夫", "妻子",
			"朋友", "亲戚", "陌生人", "敌人", "同事", "同学", "团队", "群体", "社会", "民族",
			"政治", "政府", "主席", "总理", "政党", "选举", "法律", "权力", "自由", "平等",
			"经济", "工资", "薪水", "收入", "支出", "储蓄", "银行", "投资", "贸易", "市场",
			"食物", "米饭", "面条", "馄饨", "饺子", "春卷", "粥", "粉", "糖", "盐", "油", "酱油", "醋", "酱",
			"饮料", "水", "茶", "咖啡", "牛奶", "果汁", "酒", "啤酒", "葡萄酒", "鸡尾酒", "汽水", "果酱",
...
calculateScore() {
	...
	speakBingo(source = 'player') {
		try {
			...
			try {
				this._speakWithRetry('Bingo bonus!', { lang: 'zh-CN', rate: 1.15, pitch: 1.4 }).catch(e => {
	...
	// Pick a preferred Mandarin Chinese voice if available. Returns a SpeechSynthesisVoice or null.
	_getPreferredVoice() {
		try {
			if (typeof window === 'undefined' || !('speechSynthesis' in window)) return null;
			const voices = window.speechSynthesis.getVoices() || [];
			if (!voices || voices.length === 0) return null;
			// Prefer explicit Chinese voices (zh-CN/zh-Hans) and prefer female-like names if present
			let pick = null;
			for (const v of voices) {
				if (!v.lang) continue;
				const lang = v.lang.toLowerCase();
				if ((lang.startsWith('zh') || lang.startsWith('cmn')) && (!pick || v.name.toLowerCase().includes('female') || v.name.toLowerCase().includes('female'))) {
					pick = v;
				}
			}
			if (!pick) pick = voices.find(v => v.lang && (v.lang.toLowerCase().startsWith('zh') || v.lang.toLowerCase().startsWith('cmn'))) || voices[0];
			return pick || null;
		} catch (e) {
						...
						setTimeout(async () => {
							try {
								...
								if (!window.speechSynthesis.pending && !window.speechSynthesis.speaking) {
									...
									for (let t of texts) {
										try { await this._speakWithRetry(t, { lang: 'zh-CN' }); } catch (e) { /* ignore */ }
									}
		...
		return new Promise(async (resolve) => {
			try {
				...
				for (let idx = 0; idx < words.length; idx++) {
					...
					try {
						await this._speakWithRetry(text, { lang: 'zh-CN' });
					} catch (e) {
								...
								while (attempt <= maxRetries && !success) {
									...
									try {
										...
										if (!(typeof window !== 'undefined' && 'speechSynthesis' in window && (window.speechSynthesis.speaking || window.speechSynthesis.pending))) {
											await this._speakWithRetry('Bingo bonus!', { lang: 'zh-CN' });
										}
	...
	createMoveHistoryDisplay() {
		...
		if (mobileDrawer) {
			...
			if (!consoleBox) {
				...
				consoleBox.innerHTML = '<div style="display:flex;align-items:center;justify-content:space-between"><strong>' + (typeof t === 'function' ? t('console') : '控制台') + '</strong><button class="copy-console-btn" style="font-size:12px;padding:4px 6px;border-radius:4px">' + (typeof t === 'function' ? t('copy-all') : '全部复制') + '</button></div><div class="console-entries" style="margin-top:6px;font-family:monospace"></div>';
	...
	try {
		...
		if (mobileDrawer) {
			...
			if (!consoleContainer) {
				...
				title.textContent = typeof t === 'function' ? t('console') : '控制台';
				title.style.fontWeight = '600';
				title.style.color = '#123';

				const copyBtn = document.createElement('button');
				copyBtn.textContent = typeof t === 'function' ? t('copy-all') : '全部复制';
