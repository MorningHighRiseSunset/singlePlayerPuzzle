...
class ScrabbleGame {
	constructor() {
		...
		this.preferredLang = 'fr'; // French version
...
formedWords.forEach((wordInfo) => {
	...
	async loadFrenchDictionary() {
		try {
			...
			// Filter to create a proper French Scrabble dictionary
			...
			// Additional Scrabble-specific filtering
			validWords = validWords.filter(word => {
				const lowerWord = word.toLowerCase();

				// Remove banned words (articles, pronouns, prepositions, conjunctions)
				const bannedWords = new Set([
					...
					// Prepositions
					'à', 'au', 'aux', 'dans', 'par', 'pour', 'sans', 'avec', 'chez',
					'sous', 'sur', 'entre', 'vers', 'contre', 'pendant', 'depuis', 'jusque',
					// Conjunctions
					'et', 'ou', 'mais', 'donc', 'or', 'ni', 'car', 'puisque', 'quoique',
					// Common adverbs that are too basic
					'ne', 'pas', 'plus', 'bien', 'mal', 'très', 'trop', 'peu', 'beaucoup',
					'ici', 'là', 'partout', 'nulle', 'jamais', 'toujours', 'encore', 'déjà',
					// Basic verbs in conjugated forms
					'suis', 'es', 'est', 'sommes', 'êtes', 'sont', 'étais', 'était', 'étions', 'étaient'
				]);

				if (bannedWords.has(lowerWord)) return false;

				// Remove words longer than 11 letters
				if (word.length > 11) return false;

				// Remove conjugated verb forms that are not useful for Scrabble
				if (/^(suis|es|est|sommes|êtes|sont)$/.test(lowerWord)) return false; // être conjugations
				if (/^(ai|as|a|avons|avez|ont)$/.test(lowerWord)) return false; // avoir conjugations

				return true;
			});

			// Limit to reasonable Scrabble dictionary size
			if (validWords.length > 60000) {
				validWords = validWords.slice(0, 60000);
			}

			this.frenchDictionary = new Set(validWords);
			console.log(`French Scrabble dictionary loaded: ${this.frenchDictionary.size} words`);
		} catch (error) {
