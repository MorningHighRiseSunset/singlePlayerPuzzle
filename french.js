...
class ScrabbleGame {
	constructor() {
		this.board = Array(15)
		   ...
		   this.tileValues = {
			   ...
			   É: 1, È: 1, Ê: 1, À: 1, Ç: 5, Ù: 1, Â: 1, Î: 1, Ô: 1, Ë: 1, Ï: 1, Ü: 1, Œ: 10, Æ: 10,
			   "*": 0,
		   };
		   this.tileDistribution = {
			   ...
			   É: 6, È: 2, Ê: 2, À: 1, Ç: 1, Ù: 1, Â: 1, Î: 1, Ô: 1, Ë: 1, Ï: 1, Ü: 1, Œ: 1, Æ: 1,
			   "*": 2
		   };
			...
			if (this.ghostAIMove) {
				...
				if (rackSnapshot === currentRack && boardSnapshot === currentBoard) {
					...
					const canForm = this.canFormWord(
					   ...
					   if (validity.valid && canForm && isStrictFrenchWord(word, this.dictionary)) {
			...
			if (possiblePlays && possiblePlays.length > 0) {
				...
				for (const candidate of possiblePlays) {
					...
					if (Date.now() - startTime > 60000) {
						// Only play if strictly valid French word
						   if (isStrictFrenchWord(candidate.word, this.dictionary)) {
							bestPlay = candidate;
					...
					const validity = this.checkAIMoveValidity(candidate.word, candidate.startPos, candidate.isHorizontal);
					   if (validity.valid && isStrictFrenchWord(candidate.word, this.dictionary)) {
						bestPlay = candidate;
	...
	checkAIMoveValidity(word, startPos, isHorizontal) {
		// List of variant spellings or non-French words to exclude
		const excludedVariants = new Set([
			"atropin", // German spelling, not valid in French Scrabble
...
calculateScore() {
	...
	speakBingo(source = 'player') {
		try {
			...
			try {
				this._speakWithRetry('Bingo bonus!', { lang: 'fr-FR', rate: 1.15, pitch: 1.4 }).catch(e => {
	...
	// Pick a preferred French voice if available. Returns a SpeechSynthesisVoice or null.
	_getPreferredVoice() {
		try {
			if (typeof window === 'undefined' || !('speechSynthesis' in window)) return null;
			const voices = window.speechSynthesis.getVoices() || [];
			if (!voices || voices.length === 0) return null;
			// Prefer explicit French voices (fr-FR/fr-CA) and prefer female-like names if present
			let pick = null;
			for (const v of voices) {
				if (!v.lang) continue;
				const lang = v.lang.toLowerCase();
				if (lang.startsWith('fr') && (!pick || v.name.toLowerCase().includes('female') || v.name.toLowerCase().includes('female'))) {
					pick = v;
				}
			}
			if (!pick) pick = voices.find(v => v.lang && v.lang.toLowerCase().startsWith('fr')) || voices[0];
			return pick || null;
		} catch (e) {
						...
						setTimeout(async () => {
							try {
								...
								if (!window.speechSynthesis.pending && !window.speechSynthesis.speaking) {
									...
									for (let t of texts) {
										try { await this._speakWithRetry(t, { lang: 'fr-FR' }); } catch (e) { /* ignore */ }
									}
		...
		return new Promise(async (resolve) => {
			try {
				...
				for (let idx = 0; idx < words.length; idx++) {
					...
					try {
						await this._speakWithRetry(text, { lang: 'fr-FR' });
					} catch (e) {
								...
								while (attempt <= maxRetries && !success) {
									...
									try {
										...
										if (!(typeof window !== 'undefined' && 'speechSynthesis' in window && (window.speechSynthesis.speaking || window.speechSynthesis.pending))) {
											await this._speakWithRetry('Bingo bonus!', { lang: 'fr-FR' });
										}
	...
	try {
		...
		if (mobileDrawer) {
			...
			if (!consoleContainer) {
				...
				copyBtn.textContent = typeof t === 'function' ? t('copy-all') : 'Copier Tout';
