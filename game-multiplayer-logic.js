...
// Multiplayer mode flag - disables AI and enables Socket.io sync
...
class ScrabbleGame {
	constructor() {
		...
		this.tiles = [];
		this.playerRack = [];
		this.opponentRack = []; // Changed from aiRack
		this.playerScore = 0;
		this.opponentScore = 0; // Changed from aiScore
	...
	// DISABLED: AI ghost move functionality
	showAIGhostMove(play) {
		if (IS_MULTIPLAYER) return; // Skip in multiplayer
	...
	async showAIGhostIfPlayerMoveValid() {
		...
		if (aiPlays && aiPlays.length > 0) {
			...
			this.ghostAIMove = {
				...bestPlay,
				rackSnapshot: this.opponentRack.map(t => t.letter).sort().join(''),
	...
	setupTapPlacement() {
		...
		document.getElementById("tile-rack").addEventListener("click", (e) => {
			...
			// If a tile from the board is selected, do NOT send it back to the rack here.
			// Players must use the dedicated "Return Tiles" control to undo a move.
	...
	// DISABLED: AI turn functionality - multiplayer uses Socket.io instead
	async aiTurn() {
		if (IS_MULTIPLAYER) return; // Skip in multiplayer
		this.aiValidationLogSet = new Set();
	if (this.showAIDebug) console.log("AI thinking...");
		...
		try {
			...
			if (this.ghostAIMove) {
				...
				const currentRack = this.opponentRack.map(t => t.letter).sort().join('');
				...
				if (rackSnapshot === currentRack && boardSnapshot === currentBoard) {
					...
					const canForm = this.canFormWord(
						...
						this.opponentRack.map(t => t.letter)
					);
	...
	shouldExchangeTiles() {
		...
		if (plays.length === 0) {
			console.log("AI rack:", this.opponentRack.map(t => t.letter));
	...
	findSimplePlays() {
		const plays = [];
		const rack = this.opponentRack.map((t) => t.letter);
	...
	formMinimalValidWord() {
		...
		const availableLetters = this.opponentRack.map((tile) => tile.letter);
	...
	findAIPossiblePlays() {
		...
		try {
			const possiblePlays = [];
			const rack = this.opponentRack.map(tile => tile.letter);
	...
	selectBestPlay(plays) {
		...
		const strictMode = this.tiles.length < 10 || this.opponentRack.length <= 3;
	...
	evaluateRackBalanceAfter(word) {
		...
		const tempRack = [...this.opponentRack];
	...
	canGoOut(word) {
		...
		const rackLetters = this.opponentRack.map(t => t.letter);
		const wordLetters = word.toUpperCase().split('');
		...
		const header = `
            <div class="header">
                ...
                <div class="scores">
                    ...
                    <p>Computer: ${this.opponentScore} points</p>
...
async executeAIPlay(play) {
    ...
    if (!validity.valid) {
        ...
        if (!IS_MULTIPLAYER) setTimeout(async () => this.aiTurn(), 1000);
        return;
    }
    ...
    return new Promise(async (resolve) => {
        ...
        for (let i = 0; i < word.length; i++) {
            ...
            if (!this.board[row][col]) {
                ...
                let tileIndex = this.opponentRack.findIndex((t) => t.letter === letter);
                if (tileIndex === -1) {
                    // Try to use blank tile
                    tileIndex = this.opponentRack.findIndex((t) => t.letter === "*");
                    if (tileIndex !== -1) {
                        // Convert blank tile to needed letter
                        this.opponentRack[tileIndex] = {
                            letter: letter,
                            value: 0,
                ...
                if (tileIndex !== -1) {
                    const tile = {
                        letter: letter,
                        value: this.opponentRack[tileIndex].isBlank ? 0 : this.tileValues[letter],
                        id: this.opponentRack[tileIndex].isBlank ?
                            `blank_${letter}_${Date.now()}_${i}` : `ai_${letter}_${Date.now()}_${i}`,
                        isBlank: this.opponentRack[tileIndex].isBlank,
                    };
                    ...
                    await new Promise((resolve) => {
                        setTimeout(() => {
                            ...
                            setTimeout(() => {
                                ...
                                this.opponentRack.splice(tileIndex, 1);
                                this.board[row][col] = tile;
                                this.renderAIRack();
	...
	findPossibleMoves() {
		const moves = [];
		const letters = this.opponentRack.map((tile) => tile.letter);
...
formedWords.forEach((wordInfo) => {
	...
	playAIMove(move) {
		...
		for (let i = 0; i < word.length; i++) {
			...
			const tileIndex = this.opponentRack.findIndex((t) => t.letter === letter);
			if (tileIndex !== -1) {
				const tile = this.opponentRack.splice(tileIndex, 1)[0];
	...
	balanceAIRack() {
		const vowels = ['A', 'E', 'I', 'O', 'U'];
		const vowelCount = this.opponentRack.filter(tile => vowels.includes(tile.letter)).length;

		// Aim for 2-3 vowels in the rack
		if (vowelCount < 2 || vowelCount > 4) {
			const desiredVowelCount = 3;
			while (this.tiles.length > 0 &&
				this.opponentRack.filter(tile => vowels.includes(tile.letter)).length !== desiredVowelCount) {

				// Remove excess vowels or consonants
				const indexToRemove = this.opponentRack.findIndex(tile =>
					vowelCount > 3 ? vowels.includes(tile.letter) : !vowels.includes(tile.letter)
				);

				if (indexToRemove !== -1) {
					this.tiles.push(this.opponentRack.splice(indexToRemove, 1)[0]);
					...
					if (tileIndex !== -1) {
						this.opponentRack.push(this.tiles.splice(tileIndex, 1)[0]);
					} else {
						this.opponentRack.push(this.tiles.pop());
					}
				}
			}
		}
...
calculateScore() {
	...
	checkGameEnd() {
		if (
			this.tiles.length === 0 &&
			(this.playerRack.length === 0 || this.opponentRack.length === 0)
		) {
			this.gameEnded = true;
			this.announceWinner();
			return true;
	...
	announceWinner() {
		...
		this.opponentRack.forEach(tile => {
		...
		if (this.playerRack.length === 0 && this.opponentRack.length > 0) {
			// Player used all tiles: player gets AI's leftover, AI loses their leftover
			playerScore += aiLeftover;
			aiScore -= aiLeftover;
		} else if (this.opponentRack.length === 0 && this.playerRack.length > 0) {
		...
		this.opponentScore = aiScore;

		// Update scores before animation
		this.updateScores();

		const isDraw = this.playerScore === this.opponentScore;
		const winner = isDraw ? null : (this.playerScore > this.opponentScore ? "Player" : "Computer");
		const finalScore = Math.max(this.playerScore, this.opponentScore);
		...
		messageBox.innerHTML = `
			...
			<p>Computer: ${this.opponentScore}</p>
	...
	renderAIRack() {
		const rack = document.getElementById("opponent-rack");
		rack.innerHTML = "";
		this.opponentRack.forEach((tile) => {
			const tileElement = document.createElement("div");
			tileElement.className = "tile";
	...
	fillRacks(playerFirst = false) {
		...
		const fillAI = () => {
			...
			while (this.opponentRack.length < 7 && this.tiles.length > 0) {
				// 40% chance to get exactly what AI needs
				if (Math.random() < 0.4) {
					const currentVowels = this.opponentRack.filter(tile =>
						optimalDistribution.vowels.includes(tile.letter)).length;
					...
					if (desiredTile) {
						this.opponentRack.push(desiredTile);
						continue;
					}
				}
				...
				if (tile) this.opponentRack.push(tile);
			}
		};
	...
	handleAIExchange() {
		...
		this.showAIExchangeAnimation().then(() => {
			// Get current AI rack
			const oldTiles = [...this.opponentRack];
			const exchangeCount = Math.min(this.opponentRack.length, this.tiles.length);
			...
			for (const tile of tilesToExchange) {
				const index = this.opponentRack.findIndex(t => t.letter === tile.letter);
				if (index !== -1) {
					this.opponentRack.splice(index, 1);
			...
			console.log("AI exchange complete:", {
				...
				newTiles: this.opponentRack.map((t) => t.letter),
			});
	...
	selectTilesToExchange(count) {
		...
		const scoredTiles = this.opponentRack.map(tile => {
			let score = 0;
			...
			const duplicateCount = this.opponentRack.filter(t => t.letter === tile.letter).length;
			if (duplicateCount > 1) {
	...
	async showAIExchangeAnimation() {
		...
		const aiRackElement = document.getElementById("opponent-rack");
		...
		for (let i = 0; i < this.opponentRack.length; i++) {
	...
	setupEventListeners() {
		...
		if (quitButton) {
			quitButton.addEventListener("click", () => {
				...
				this.opponentScore = Math.max(this.opponentScore, this.playerScore + 1);
				this.playerScore = Math.min(this.playerScore, this.opponentScore - 1);
				this.gameEnded = true;
