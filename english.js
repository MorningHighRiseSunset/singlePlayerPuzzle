				...
				if (mainWord && mainWord.length > 1) {
					...
					if (validation.isValid && !excludedVariants.has(mainWord.toLowerCase())) {
						...
						if (mainWord.length === 2) {
							...
							const inAnyDict = (this.dictionary && this.dictionary.has(lw));
...
formedWords.forEach((wordInfo) => {
	...
	async loadDictionary() {
		this.updateLoadingProgress('Loading English dictionary (nlile)...');
		try {
			// Load ONLY the nlile dictionary source
			...
			// Fetch the nlile dictionary
			const response = await fetch(NLILE_URL, { cache: 'no-store' });
			if (response && response.ok) {
				dictionaryText = await response.text();
				if (this.showAIDebug) console.log(`✓ nlile dictionary loaded`);
			} else {
				throw new Error("Failed to load nlile dictionary");
			}

			if (!dictionaryText || dictionaryText.length === 0) {
				throw new Error("nlile dictionary is empty");
			}

			this.updateLoadingProgress(`Processing nlile dictionary...`);
			...
			console.log(`Tier 1 Fast-Path built: ${this.coreValidDictionary.size} high-confidence words from nlile dictionary (instant validation)`);
    ...
    async validateWordInContext(word, wordInfo, isMainWord) {
        ...
        if (this.preferredLang === 'es') {
            ...
            // For English, check ALL dictionaries (activeDictionary, dictionary, coreValidDictionary)
